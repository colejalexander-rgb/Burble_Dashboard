...
function makeCard(board) {
  ...
  frame.src = board.url;
  card.querySelector(".reload").onclick = () => {
...
reloadAll.onclick = () => document.querySelectorAll(".viewport").forEach((frame) => {
  const separator = frame.src.includes("?") ? "&" : "?";
  frame.src = `${frame.src.split("&reload=")[0]}${separator}reload=${Date.now()}`;
});
manage.onclick = () => {
